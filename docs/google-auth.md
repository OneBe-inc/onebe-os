...
確認済み: 公開ログイン画面、管理者本人の実Googleログイン、ダッシュボード表示、再読み込み後のログイン維持、保護ページのログインへの転送、保護APIの401、別オリジンからのログイン開始の403。ログアウト・未登録アカウント拒否・失効は自動テストで検証済みです（本番の実アカウントによるログアウトと未登録拒否は未実施）。
...
プロフィール画像は署名検証済みIDトークンの `picture` から取得し、許可済みメンバーの `avatar_url` に保存します（`0002_member_avatar.sql`）。GoogleのHTTPS画像ホストに限定し、画像なし・読み込み失敗時はイニシャルに戻ります。既存ユーザーは一度ログインし直すと反映され、以後ログインごとに更新します。追加スコープ・アクセストークン保存は不要です。
